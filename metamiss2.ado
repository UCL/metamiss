...
program metamiss2
version 13
	syntax [anything] [if] [in], [IMPType(string) IMPMean(string) IMPSD(string) IMPCorrelation(string) MD SMD ROM OR RR RD SDPool(string) Bootstrap Taylor REPS(string) NOKEEP NOMETA EFORM LOG FIXED COMPare(string asis) VARCHange INConsistency SENSitivity NETPLOT TRTLABels(string asis) NETPLOTREFerence(string asis) NETPLOTOPTions(string asis) METANOPTions(string asis) NETWORKOPTions(string asis)]	
	...
	if "`network'"=="on"{
		...
		if "`method'"=="bootstrap"{
			forvalues i=1/`nall'{
				tempname tn`i' tm`i' tmean`i' tsd`i' tR`i' z`i' Corr`i' MLcorr`i' SLcorr`i' SLcov`i'
			}
	...
	if "`network'"=="on"{
		...
		/*run a sensitivity analysis using a range of standard deviations for IMP*/
			...
			di as text "********                IMP with uncertainty               ********"
		  ...
		  /*run network meta-analysis using the estimated 'true' relative effects, variances, covariances*/
			...
			if _rc==0{
				di as text "********               IMP without uncertainty             ********"
			}
			cap{
				assert  `tayl'==0
			}
			...
			if "`nometa'"==""{
				...
				if _rc==0{
					if "$METAMISS2_compare"!=""{
						di as text "with options: $METAMISS2_options" _c
					}
					else{
	...
	if "`network'"=="off"{
		...
		/*run a sensitivity analysis using a range of standard deviations for the IMP*/
			...
			if _rc==0{
				di as text "Priors used:" _col(36) _c
				di as result "Contol group ~ N(`mulambdac',`sdlambdac'^2)"
		...
		cap scalar drop Mreps Mns Mod Meas Dat
...
program def compare
	...
	if "$METAMISS2_network"=="off"{
		...
		if "$METAMISS2_method" =="taylor"{
			if "$METAMISS2_type"=="bin"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, metanopt(notable nograph $METAMISS2_options by(`double') nooverall) $METAMISS2_measure $METAMISS2_eform impmean("$METAMISS2_impmean") impsd("$METAMISS2_impsd") impcorr("$METAMISS2_corr")  
			}
			if "$METAMISS2_type"=="cont"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, metanopt(notable nograph $METAMISS2_options by(`double') nooverall) $METAMISS2_measure $METAMISS2_eform imptype("$METAMISS2_model") impmean("$METAMISS2_impmean") impsd("$METAMISS2_impsd") impcorr("$METAMISS2_corr")  
			}
		}
		if "$METAMISS2_method" =="bootstrap"{
			if "$METAMISS2_type"=="bin"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, metanopt(notable nograph $METAMISS2_options by(`double') nooverall) $METAMISS2_measure $METAMISS2_eform b reps("$METAMISS2_reps") impmean("$METAMISS2_impmean") impsd("$METAMISS2_impsd") impcorr("$METAMISS2_corr")  		
			}
			if "$METAMISS2_type"=="cont"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, metanopt(notable nograph $METAMISS2_options by(`double') nooverall) $METAMISS2_measure $METAMISS2_eform b reps("$METAMISS2_reps") imptype("$METAMISS2_model") impmean("$METAMISS2_impmean") impsd("$METAMISS2_impsd") impcorr("$METAMISS2_corr")  		
			}
		} 
		...
		if _rc!=0{
			...
			if "$METAMISS2_type"=="bin"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, metanopt(notable nograph $METAMISS2_options by(`double') nooverall) impmean(0) impsd(0)  $METAMISS2_eform $METAMISS2_measure 
			}
			if "$METAMISS2_type"=="cont"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, metanopt(notable nograph $METAMISS2_options by(`double') nooverall) impmean(0) impsd(0)  $METAMISS2_eform $METAMISS2_measure imptype("$METAMISS2_model") 
			}
		}
		else{
			if "$METAMISS2_type"=="bin"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, metanopt(notable nograph $METAMISS2_options by(`double') nooverall) $METAMISS2_compare  $METAMISS2_eform $METAMISS2_measure 
			}
			if "$METAMISS2_type"=="cont"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, metanopt(notable nograph $METAMISS2_options by(`double') nooverall) $METAMISS2_compare  $METAMISS2_eform $METAMISS2_measure imptype("$METAMISS2_model") 
			}
		}
		...
		dicmd metan `es' `sees', random $METAMISS2_options by(`double') nooverall $METAMISS2_eform
	...
	if "$METAMISS2_network"=="on"{
		...
		if "$METAMISS2_method" =="taylor"{
			if "$METAMISS2_type"=="bin"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, $METAMISS2_measure impmean("$METAMISS2_impmean") impsd("$METAMISS2_impsd") impcorr("$METAMISS2_corr") networkopt($METAMISS2_options) $METAMISS2_inc
			}
			if "$METAMISS2_type"=="cont"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, $METAMISS2_measure imptype("$METAMISS2_model") impmean("$METAMISS2_impmean") impsd("$METAMISS2_impsd") impcorr("$METAMISS2_corr") networkopt($METAMISS2_options) $METAMISS2_inc
			}
		}
		if "$METAMISS2_method" =="bootstrap"{
			if "$METAMISS2_type"=="bin"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, $METAMISS2_measure b reps("$METAMISS2_reps") impmean("$METAMISS2_impmean") impsd("$METAMISS2_impsd") impcorr("$METAMISS2_corr") networkopt($METAMISS2_options) $METAMISS2_inc		 
			}
			if "$METAMISS2_type"=="cont"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, $METAMISS2_measure b reps("$METAMISS2_reps") imptype("$METAMISS2_model") impmean("$METAMISS2_impmean") impsd("$METAMISS2_impsd") impcorr("$METAMISS2_corr") networkopt($METAMISS2_options) $METAMISS2_inc		
			}
		} 
		if "$METAMISS2_plot"!=""{
		...
		if _rc!=0{
			di as text _newline " {bf:Warning}: Option '$METAMISS2_compare' in {it:compare()} is not a valid IMP definition - ACA is used as the comparator model" 
			if "$METAMISS2_type"=="bin"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, impmean(0) impsd(0) networkopt($METAMISS2_options) $METAMISS2_eform
			}
			if "$METAMISS2_type"=="cont"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, impmean(0) impsd(0) networkopt($METAMISS2_options) $METAMISS2_eform imptype("$METAMISS2_model")
			}
		}
		else{
			if "$METAMISS2_type"=="bin"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, $METAMISS2_compare networkopt($METAMISS2_options) $METAMISS2_eform
			}
			if "$METAMISS2_type"=="cont"{
				metamiss2 $METAMISS2_anything $METAMISS2_if $METAMISS2_in, $METAMISS2_compare networkopt($METAMISS2_options) $METAMISS2_eform imptype("$METAMISS2_model")
			}
		}
		if "$METAMISS2_plot"!=""{
...
program def sensitivity
	...
	if "$METAMISS2_network"=="off"{
		...
		sc `ESlambda' `sdlambda', mcol(black)|| sc `LCIlambda' `sdlambda',mcol(black) msymb(plus) || sc `UCIlambda' `sdlambda', mcol(black) msymb(plus) legend(off) xtitle("Standard deviation of `xtitle'") ytitle(`ytitle') `options'
		cap drop _LCI _UCI _WT
	}
	if "$METAMISS2_network"=="on"{
