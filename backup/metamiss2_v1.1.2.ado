*! version 1.1.2, 2.3.2015, Anna Chaimani & Ian White

program metamiss2
	syntax varlist(min=8 max=8) [if] [in], [IMPType(string) IMPMean(string) IMPSD(string) IMPCorrelation(string) MD SMD ROM Bootstrap Taylor REPS(string) NOKEEP FIXED COMPare(string asis) SENSitivity *]
	...
	if "`imptype'"=="imdom" | "`imptype'"==""{
		...
		if "`impmean'"!="" | "`impsd'"!=""{
			di as text _newline " {bf:Warning}: Values in options {it:impmean()} and {it:impsd()} should be given in logarithmic scale for the IMROM model"
		}
		cap{
			assert `yt'>=0 & `yc'>=0
		}
	...
	qui{
		...
		if "`method'"=="taylor"{ //estimation via Taylor approximation
			if "`model'"=="imrom"{
				...
				gen `sumc'=((`pc'*(1-`pc'))/(`nc'+`mc'))*(1-2*exp(`mulc'+0.5*`sdlc'^2)+exp(2*`mulc'+2*`sdlc'^2))*`yc'^2+(`sdc'^2/`nc')*((1-`pc')^2*exp(2*`mulc'+2*`sdlc'^2)+2*`pc'*(1-`pc')*exp(`mulc'+0.5*`sdlc'^2)+`pc'^2)+exp(2*`mulc'+`sdlc'^2)*(exp(`sdlc'^2)-1)*((1-`pc')*`yc')^2 `if' `in'
				gen `sumt'=((`pt'*(1-`pt'))/(`nt'+`mt'))*(1-2*exp(`mult'+0.5*`sdlt'^2)+exp(2*`mult'+2*`sdlt'^2))*`yt'^2+(`sdt'^2/`nt')*((1-`pt')^2*exp(2*`mult'+2*`sdlt'^2)+2*`pt'*(1-`pt')*exp(`mult'+0.5*`sdlt'^2)+`pt'^2)+exp(2*`mult'+`sdlt'^2)*(exp(`sdlt'^2)-1)*((1-`pt')*`yt')^2	`if' `in'		
			}
			if "`model'"=="imdom"{
				...
				gen `sumc'=((`pc'*(1-`pc'))/(`nc'+`mc'))*(`mulc'^2+`sdlc'^2)+(`sdc'^2/`nc')+`sdlc'^2*(1-`pc')^2 `if' `in'
				gen `sumt'=((`pt'*(1-`pt'))/(`nt'+`mt'))*(`mult'^2+`sdlt'^2)+(`sdt'^2/`nt')+`sdlt'^2*(1-`pt')^2 `if' `in'
			}
			if "`measure'"=="md"{
				gen `f1'=1 `if' `in'
				gen `f2'=1 `if' `in'
...
mac list impmean
dicmd	metamiss2 $varlist $if $in, notable nograph $compare by(`double') nooverall $options
mac list impmean
	...
	if "$method" =="taylor"{
dicmd		metamiss2 $varlist $if $in, notable nograph $measure imptype("$model") impmean("$impmean") impsd("$impsd") corr("$corr") $options by(`double') nooverall
	}
	if "$method" =="bootstrap"{
dicmd		metamiss2 $varlist $if $in, notable nograph $measure t reps("$reps") imptype("$model") impmean("$impmean") impsd("$impsd") corr("$corr") $options by(`double') nooverall
	}
...
program def sensitivity
	...
	forvalues i=2/100{
		...
		if "$method" =="taylor"{
			qui metamiss2 $varlist $if $in, notable nokeep nograph "$measure" `sd' impmean("$impmean") corr("$corr")
		}
		if "$method" =="bootstrap"{
			qui metamiss2 $varlist $if $in, notable nokeep nograph "$measure" b reps("$reps") `sd' impmean("$impmean") corr("$corr")
		}
		local ES`i'=r(ES)
