...
quiet{
    foreach reason in 0 1 E C P imor {
        gen `aEsup`reason'' = `aE' * (`umE`reason'' / (`umE0'+`umE1'+`umEE'+`umEC'+`umEP'+`umEimor')) if `nEreas'~=0
        gen `aCsup`reason'' = `aC' * (`umC`reason'' / (`umC0'+`umC1'+`umCE'+`umCC'+`umCP'+`umCimor')) if `nCreas'~=0
...
quiet {
    ...
    replace `pEfromimorE' = (`pE'*`uimorE') / (`pE'*`uimorE' + 1 - `pE') if `umEimor'~=0
    replace `pCfromimorC' = (`pC'*`uimorC') / (`pC'*`uimorC' + 1 - `pC') if `umCimor'~=0

    gen `pEstar' = `pE'*(1-`aE') + `aEsup1' + `aEsupC'*`pC' + `aEsupE'*`pE' + `aEsupP'*`pE' + `aEsupimor'*`pEfromimorE'
    gen `pCstar' = `pC'*(1-`aC') + `aCsup1' + `aCsupC'*`pC' + `aCsupE'*`pE' + `aCsupP'*`pC' + `aCsupimor'*`pCfromimorC'
    `aca' replace `pEstar' = `pE'
    `aca' replace `pCstar' = `pC'
    replace `pEstar' = `pE' if `defaulttoacaE' == 1
    replace `pCstar' = `pC' if `defaulttoacaC' == 1
    }


* create new `effective' 2x2 table based on new overall risks, incorporating missing data imputations, with original sample size

tempvar erE efE erC efC effrE efffE effrC efffC
tempvar enE enC epEstar epCstar

quiet {
    gen `effrE' = `nE' * `pEstar'
    gen `efffE' = `nE' * (1-`pEstar')
    gen `effrC' = `nC' * `pCstar'
    gen `efffC' = `nC' * (1-`pCstar')
}


* adjust cells with zeros in 'effective' 2x2 table
quiet {
    tempvar anyzero
    gen `anyzero' = min(`effrE', `efffE', `effrC', `efffC')==0
    gen `erE' = `effrE'+`anyzero'/2
    gen `efE' = `efffE'+`anyzero'/2
    gen `erC' = `effrC'+`anyzero'/2
    gen `efC' = `efffC'+`anyzero'/2                     /* IW shortened */

* new sample sizes and probabilities taking into account 0.5s for zero cells
    gen `enE' = `erE' + `efE'
    gen `enC' = `erC' + `efC'
...
if "`listp'"=="listp" {
    char `pE'[varname] pe
    char `epEstar'[varname] epEstar
    char `pC'[varname] pC
    char `epCstar'[varname] epCstar
    list `id' `pE' `epEstar' `pC' `epCstar', subvarname
}
...
quiet{
   if "`measure'"=="RD" gen `eststar' = `epEstar' - `epCstar'
   else if "`measure'"=="RR" gen `eststar' = log(`epEstar'/`epCstar')
   else if "`measure'"=="OR" gen `eststar' = log((`epEstar'/(1-`epEstar'))/(`epCstar'/(1-`epCstar')))
}

* create new counts and probabilities if there's a zero cell: 2x2 counts and probabilities ignore missing data
tempvar xrE xfE xrC xfC xnE xnC xpE xpC
quiet{
   foreach count in rE fE rC fC {
      gen `x`count'' = ``count''
      replace `x`count'' = ``count''+0.5 if (`rE'==0 & `fE'~=0 & `rC'~=0 & `fC'~=0 | `rE'~=0 & `fE'==0 & `rC'~=0 & `fC'~=0 | `rC'==0 & `fC'~=0 & `rE'~=0 & `fE'~=0  | `rC'~=0 & `fC'==0 & `rE'~=0 & `fE'~=0 )
   }
   gen `xnE'=`xrE'+`xfE'+`umE'
   gen `xnC'=`xrC'+`xfC'+`umC'
   gen `xpE' = `xrE'/(`xrE'+`xfE')
...
if "`weight'"=="w1" {
   if "`rd'"=="rd" gen `sestar' = sqrt(`erE'*(`enE'-`erE')/(`enE')^3 + `erC'*(`enC'-`erC')/(`enC')^3 )
   if "`rr'"=="rr" gen `sestar' = sqrt(1/`erE' - 1/`enE' + 1/`erC' - 1/`enC')
   if "`or'"=="or" gen `sestar' = sqrt(1/`erE' + 1/(`enE'-`erE') + 1/`erC' + 1/(`enC'-`erC'))
}
else if "`weight'"=="w2" {
   * ACA fixed-effect analysis for standard errors
   quiet metan `rE' `fE' `rC' `fC', fixedi `or' `rr' `rd' nointeger
   if "`rd'"=="rd" gen `sestar' = _seES
   else gen `sestar' = _selogES
}
else if "`weight'"=="w3" {
   * apply pstar probabilities to original ACA sample sizes (after zero cell adjustment)
   ...
   gen `W3rE' = `epEstar' * (`xrE'+`xfE')
   gen `W3rC' = `epCstar' * (`xrC'+`xfC')
   gen `W3fE' = (1-`epEstar') * (`xrE'+`xfE')
   gen `W3fC' = (1-`epCstar') * (`xrC'+`xfC')
   if "`rd'"=="rd" gen `sestar' = sqrt(`W3rE'*`W3fE'/(`xrE'+`xfE')^3 + `W3rC'*`W3fC'/(`xrC'+`xfC')^3)
   if "`rr'"=="rr" gen `sestar' = sqrt(1/`W3rE' - 1/(`xrE'+`xfE') + 1/`W3rC' - 1/(`xrC'+`xfC'))
   if "`or'"=="or" gen `sestar' = sqrt(1/`W3rE' + 1/`W3fE' + 1/`W3rC' + 1/`W3fC')
}
else if "`weight'"=="w4" {
   ...
   * formulae use 0-cell adjusted PSTARs, Ns, Ps
   ...
   gen `lastsumE' = `aEsup0' * (`epEstar'^2 - (`xpE'-`epEstar')^2) + `aEsup1' * ((1-`epEstar')^2 - (`xpE'-`epEstar')^2) + `aEsupC' * ((`xpC'-`epEstar')^2 - (`xpE'-`epEstar')^2) + `aEsupimor' * ((`pEfromimorE'-`epEstar')^2 - (`xpE'-`epEstar')^2)
   gen `lastsumC' = `aCsup0' * (`epCstar'^2 - (`xpC'-`epCstar')^2) + `aCsup1' * ((1-`epCstar')^2 - (`xpC'-`epCstar')^2) + `aCsupE' * ((`xpE'-`epCstar')^2 - (`xpC'-`epCstar')^2) + `aCsupimor' * ((`pCfromimorC'-`epCstar')^2 - (`xpC'-`epCstar')^2)

   gen `varpEstar' = (`xpE' * (1-`xpE') / (`xnE' - `umE') ) * (1 + `firstsumE')^2 + (1/`xnE') * ((`xpE'-`epEstar')^2 + `lastsumE')
   gen `varpCstar' = (`xpC' * (1-`xpC') / (`xnC' - `umC') ) * (1 + `firstsumC')^2 + (1/`xnC') * ((`xpC'-`epCstar')^2 + `lastsumC')

    if "`rd'"=="rd" gen `sestar' = sqrt( cond( `epEstar'==0, 0, `varpEstar') +  cond(`epCstar'==0, 0, `varpCstar'))

    if "`rr'"=="rr" gen `sestar' = sqrt( cond( `epEstar'==0, 0, `varpEstar'/(`epEstar'^2)) +  cond(`epCstar'==0, 0, `varpCstar'/(`epCstar'^2)))

    if "`or'"=="or" gen `sestar' = sqrt( cond( `epEstar'==0, 0, `varpEstar'/((`epEstar'*(1-`epEstar'))^2) ) + cond(`epCstar'==0, 0, `varpCstar'/((`epCstar'*(1-`epCstar'))^2) ) )
    }
...
if `nmeasures'==1 {
    ...
    metan `eststar' `sestar' `if' `in', `idopt' `options' `eform' nograph
    qui gen _ES = `eststar'
