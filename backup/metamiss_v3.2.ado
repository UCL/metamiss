...
if "`weight'"=="w1" {
   ...
   * formulae use 0-cell adjusted PSTARs, Ns, Ps
   ...
   gen `varpCstar' = (`xpC' * (1-`xpC') / (`xnC' - `umC') ) * (1 + `firstsumC')^2 + (1/`xnC') * ((`xpC'-`epCstar')^2 + `lastsumC')
    ...
    if "`measure'"=="RD" gen `sestar' = sqrt( cond( `epEstar'==0, 0, `varpEstar') +  cond(`epCstar'==0, 0, `varpCstar'))

    if "`measure'"=="RR" gen `sestar' = sqrt( cond( `epEstar'==0, 0, `varpEstar'/(`epEstar'^2)) +  cond(`epCstar'==0, 0, `varpCstar'/(`epCstar'^2)))

    if "`measure'"=="OR" gen `sestar' = sqrt( cond( `epEstar'==0, 0, `varpEstar'/((`epEstar'*(1-`epEstar'))^2) ) + cond(`epCstar'==0, 0, `varpCstar'/((`epCstar'*(1-`epCstar'))^2) ) )
}
