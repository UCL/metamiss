*! version 1.1, 2.3.2015, Anna Chaimani & Ian White

program metamiss2
	syntax varlist(min=8 max=8) [if] [in], [IMPType(string) IMPMean(string) IMPSD(string)
CORRelation(string) MD SMD ROM Bootstrap Taylor REPS(string) NOKEEP FIXED COMPare(string asis)
SENSitivity *]
	
	tokenize `varlist'
	
	/*input data: observed, unobserved, mean, sd in treatment and control arm - consistent with
metan*/
	local nt `1'
	local mt `2'
	local yt `3'
	...
	else if "`imptype'"=="imrom"{
		local model "imrom" //model IMRoM
		if "`impmean'"!="" | "`impsd'"!=""{
			di as text _newline " {bf:Warning}: Values in options {it:impmean()} and {it:impsd()}
should be given in logarithmic scale for the IMROM model"
		}
		cap{
			assert `yt'>=0 & `yc'>=0
		}
		if _rc!=0{
			...
			if _rc!=0{
				di as err "Effect sizes with opposite signs cannot be incorporated in the {it:imrom}
model"
				exit
			}
		}		
	...
	if "`impmean'"!=""{
		...
		if "`3'"!=""{
			di as err "Option {it:impmean()} requires 1 or 2 input values/variables"
			exit
		}		
	}
	if "`impsd'"!=""{
	...
	/*correlation of IMP in treatment and control arms, default is corr(0)*/
	if "`correlation'"!=""{
	...
	if "`bootstrap'"==""{
		local method "taylor" //estimate relative effects and variances using parametric bootstrap,
default
	}
	if "`bootstrap'"!=""{
	...
	if "`sdlambdat'"=="0" & "`sdlambdac'"=="0" & "`sensitivity'"==""{
		local method "taylor"
		if "`bootstrap'"!=""{
			di _newline " Note: Option {it:bootstrap} is not allowed when sd(IMDOM)/sd(IMROM) is zero
for both experimental and control arms - reset to default option ({it:taylor})"
		}
	}
	...
	if "`rom'"!="" & "`smd'"=="" & "`md'"==""{
		...
		if _rc!=0{
			...
			if _rc!=0{
				di as err "Effect sizes with opposite signs cannot be used when ratios of means are
estimated"
				exit
			}
		}		
	...
	qui{
		...
		if "`method'"=="taylor"{ //estimation via Taylor approximation
			if "`model'"=="imrom"{
				gen `ytotc'=`pc'*`yc'+(1-`pc')*(exp(`mulc'+(`sdlc'^2)/2)*`yc') `if' `in' //estimate of
'true' mean in control arm
				gen `ytott'=`pt'*`yt'+(1-`pt')*(exp(`mult'+(`sdlt'^2)/2)*`yt') `if' `in' //estimate of
'true' mean in treatment arm
				gen
`covf'=`corrl'*`sdlt'*`sdlc'*(1-`pt')*(1-`pc')*`yt'*`yc'*exp(`mult'+0.5*`sdlt'^2)*exp(`mulc'+0.5*`sdlc'^2)
`if' `in'
				gen
`sumc'=((`pc'*(1-`pc'))/(`nc'+`mc'))*(1-2*exp(`mulc'+0.5*`sdlc'^2)+exp(2*`mulc'+2*`sdlc'^2))*`yc'^2+(`sdc'^2/`nc')*((1-`pc')^2*exp(2*`mulc'+2*`sdlc'^2)+2*`pc'*(1-`pc')*exp(`mulc'+0.5*`sdlc'^2)+`pc'^2)+exp(2*`mulc'+`sdlc'^2)*(exp(`sdlc'^2)-1)*((1-`pc')*`yc')^2
`if' `in'
				gen
`sumt'=((`pt'*(1-`pt'))/(`nt'+`mt'))*(1-2*exp(`mult'+0.5*`sdlt'^2)+exp(2*`mult'+2*`sdlt'^2))*`yt'^2+(`sdt'^2/`nt')*((1-`pt')^2*exp(2*`mult'+2*`sdlt'^2)+2*`pt'*(1-`pt')*exp(`mult'+0.5*`sdlt'^2)+`pt'^2)+exp(2*`mult'+`sdlt'^2)*(exp(`sdlt'^2)-1)*((1-`pt')*`yt')^2	
`if' `in'		
			}
			if "`model'"=="imdom"{
				gen `ytotc'=`pc'*`yc'+(1-`pc')*(`mulc'+`yc') `if' `in' //estimate of 'true' mean in
control arm
				gen `ytott'=`pt'*`yt'+(1-`pt')*(`mult'+`yt') `if' `in' //estimate of 'true' mean in
treatment arm
				gen `covf'=`corrl'*`sdlt'*`sdlc'*(1-`pt')*(1-`pc') `if' `in'
				gen
`sumc'=((`pc'*(1-`pc'))/(`nc'+`mc'))*(`mulc'^2+`sdlc'^2)+(`sdc'^2/`nc')+`sdlc'^2*(1-`pc')^2 `if' `in'
				gen
`sumt'=((`pt'*(1-`pt'))/(`nt'+`mt'))*(`mult'^2+`sdlt'^2)+(`sdt'^2/`nt')+`sdlt'^2*(1-`pt')^2 `if' `in'
			}
			if "`measure'"=="md"{
				gen `f1'=1 `if' `in'
				gen `f2'=1 `if' `in'
			...
			gen `vartot'=`f2'*(`sumc'*`f3'+`sumt'*`f4')-2*`f1'*`covf' `if' `in' //estimate of 'true'
variances depending on model and measure
	...
	if "`method'"=="bootstrap"{	//estimation via parametric bootstrap	
		...
		/*get the estimated 'true' relative effects and variances from the function random() - end of
code - and store them in the dataset*/
		mata random()
	...
	/*run a sensitivity analysis using a range of standard deviations for the missing parameter*/
		...
		/*run a meta-analysis on the estimated 'true' relative effects*/
			...
			if _rc==0{
				di as text "Priors used: Experimental group: N(`mulambdat',`sdlambdat'^2); Control
group: N(`mulambdac',`sdlambdac'^2); Correlation: `corr'"
			}
	...
	if (mod==2){
		...
		for (i=1; i<=rows(YMC); i++){
			for (j=1; j<=cols(YMC); j++){
				YMC[i,j]=exp(LC[i,j])*YC[i,j] //estimate the 'true' means from missing participants
for treatment and control arm from IMRoM model
				YMT[i,j]=exp(LT[i,j])*YT[i,j]
			}	
		}
	}
	if (mod==1){	
		YMC=LC+YC //estimate the 'true' means from missing participants for treatment and control arm
from IMDoM model
		YMT=LT+YT	
	}
	...
	for (i=1; i<=rows(YCtrue); i++){
		for (j=1; j<=cols(YCtrue); j++){
			YCtrue[i,j] = PC[i,j]*YC[i,j]+(1-PC[i,j])*YMC[i,j] //estimate the 'true' means from all
participants for treatment and control arm
	...
	if (meas==2){
		...
		for (i=1; i<=rows(spooled); i++){
			for (j=1; j<=cols(spooled); j++){			
				spooled[i,j]=sqrt(((NT[i,j]-1)*sdt[1,j]^2+(NC[i,j]-1)*sdc[1,j]^2)/(NT[i,j]+NC[i,j]-2))
//estimate of Spooled
			}
		}
...
program def compare
	...
	if "$method" =="taylor"{
		metamiss2 $varlist $if $in, notable nograph $measure imptype("$model") impmean("$impmean")
impsd("$impsd") corr("$corr") $options by(`double') nooverall
	}
	if "$method" =="bootstrap"{
		metamiss2 $varlist $if $in, notable nograph $measure t reps("$reps") imptype("$model")
impmean("$impmean") impsd("$impsd") corr("$corr") $options by(`double') nooverall
	}
...
program def sensitivity
	...
	qui metamiss2 $varlist $if $in, notable nokeep nograph "$measure" impmean("$impmean")
	local ES1=r(ES)
	local LCI1=r(ci_low)
	local UCI1=r(ci_upp)	
	forvalues i=2/100{
		...
		if "$method" =="taylor"{
			qui metamiss2 $varlist $if $in, notable nokeep nograph "$measure" `sd' impmean("$impmean")
corr("$corr")
		}
		if "$method" =="bootstrap"{
			qui metamiss2 $varlist $if $in, notable nokeep nograph "$measure" b reps("$reps") `sd'
impmean("$impmean") corr("$corr")
		}
		local ES`i'=r(ES)
	...
	sc `ESlambda' `sdlambda', mcol(black)|| sc `LCIlambda' `sdlambda',mcol(black) msymb(plus) || sc
`UCIlambda' `sdlambda', mcol(black) msymb(plus) legend(off) xtitle("Standard deviation of `xtitle'")
ytitle(`ytitle')
	cap drop if _n>`nobs'
